...
public class CarSqliteRepository extends SqliteRepository<Car> {
	...
	protected String getInsertQuery() {
		// TODO Auto-generated method stub
		return "INSERT INTO Car (Model, Year, Color) values (?,?,?)";
	}
	...
	protected void setInsertParameters(Car model, PreparedStatement insert) {
		insert.setString(1, model.getModel());
		insert.setInt(2, model.getYear());
		insert.setString(3, model.getColor());
		
	}

	@Override @SneakyThrows
	protected void fillModelFromResultSet(Car model, ResultSet result) {
		model.setId(result.getInt("id"));
		model.setModel(result.getString("model"));
		model.setYear(result.getInt("year"));
		model.setColor(result.getString("color"));
